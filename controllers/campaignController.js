...
module.exports = (io) => {
  ...
  exports.createCampaign = async (req, res) => {
    try {
      ...
      // Notify all players that assets have been updated
      if (io) {
        io.emit('assets-updated', { type: 'campaign', action: 'create', id: campaignId });
        console.log('[SOCKET] Emitted assets-updated for new campaign');
      }
  ...
  exports.updateCampaignStatus = async (req, res) => {
    try {
      ...
      // Notify all players
      if (io && (status === 'APPROVED' || status === 'approved')) {
        io.emit('assets-updated', { type: 'campaign', action: 'approve', id });
        console.log('[SOCKET] Emitted assets-updated for campaign approval');
      }
  ...
  exports.updateBillboardStatus = async (req, res) => {
    try {
      ...
      // Notify all players if a billboard within a campaign is approved
      if (io && (status === 'APPROVED' || status === 'approved')) {
        io.emit('assets-updated', { type: 'campaign-billboard', action: 'approve', campaignId, billboardId });
        console.log('[SOCKET] Emitted assets-updated for campaign billboard approval');
      }
  ...
  exports.deleteCampaign = async (req, res) => {
    try {
      ...
      // Notify all players
      if (io) {
        io.emit('assets-updated', { type: 'campaign', action: 'delete', id });
        console.log('[SOCKET] Emitted assets-updated for campaign deletion');
      }
  ...
  exports.deleteBillboardFromCampaign = async (req, res) => {
    try {
      ...
      // Notify all players
      if (io) {
        io.emit('assets-updated', { type: 'campaign-billboard', action: 'delete', campaignId, billboardId });
        console.log('[SOCKET] Emitted assets-updated for campaign billboard deletion');
      }
